...
const FILE_NAME = "migrainelog.json";
...
async function pushBlob(blob) {
  ...
  const boundary = "mlog" + Math.random().toString(16).slice(2);
  const body =
...
export async function pull() {
  ...
  for (const [name, content] of Object.entries(blob.files)) {
    if (name.startsWith("migraine_") && name.endsWith(".json")) {
...
export async function push() {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (k && k.startsWith("migraine_") && k.endsWith(".json")) files[k] = localStorage.getItem(k);
  }
