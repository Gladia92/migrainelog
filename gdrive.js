...
const FILE_NAME = "migrainelog.json"; // dans appDataFolder
...
function resultPage(ok) {
  ...
  const sub   = ok
    ? "Tu peux fermer cet onglet et revenir à <strong>MigraineLog</strong>."
    : "Aucun accès accordé. Tu peux fermer cet onglet.";
  return `<!doctype html><html lang="fr"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1"><title>MigraineLog</title></head>
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(0, "127.0.0.1", () => {
      ...
      server.on("request", async (req, res) => {
        ...
        // Ignore les requêtes parasites (favicon, etc.) qui n'ont ni code ni erreur
...
async function push(blob) {
  ...
  const boundary = "mlog" + crypto.randomBytes(8).toString("hex");
  const body =
